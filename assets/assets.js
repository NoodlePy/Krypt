...
function createAsset(crypto,value){
    ...
    const asset_cont = document.createElement("div")
        ...
        del_btn.addEventListener("click",function(){
            localStorage.removeItem("kr_" + crypto)
...
function loadAssets() {
    ...
    Object.keys(localStorage).forEach(key => {
        if (key.includes("kr_")){
            new_Key = key.replace("kr_" , "")
            createAsset(new_Key,localStorage.getItem(key))
        }
    });
}
...
add_btn.addEventListener("click", function() {
    if (inp_value.value && !(inp_value.value == 0)) {
        if (!(localStorage.getItem("kr_" + inp_crypto.value))){
            kr_string = "kr_" + inp_crypto.value
            window.localStorage.setItem(kr_string,inp_value.value)
            createAsset(inp_crypto.value,inp_value.value)
            console.log(localStorage)
        } else {
            console.log("key alr exists")
            localStorage.setItem("kr_" + inp_crypto.value , +localStorage.getItem("kr_" + inp_crypto.value)+(+inp_value.value))
            console.log(localStorage.getItem("kr_" + inp_crypto.value))
            updateAssets()
        }
    }
})